    ...
    if (pathname === '/api/purchase-orders' && method === 'POST') {
      ...
      for (const item of body.items) {
        ...
        const isWindow = (item.paper_type === 'PVC Window Film');
          ...
          const estimatedTotal = isWindow ? Math.round(rateKg * pkts) : Math.round(weightPerPkt * rateKg * pkts);
    ...
    if (editPOMatch && method === 'PUT') {
      ...
      if (po.status === 'ordered') {
        ...
        const isWindow = (po.paper_type === 'PVC Window Film');
          ...
          const newTotal = isWindow ? Math.round(po.rate_per_kg * po.received_pkts) : Math.round(weightPerPkt * po.rate_per_kg * po.received_pkts);
